      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold text-navy-900 flex items-center gap-2">
            ...
            <span>Drug Knowledge Database</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Authoritative medication monographs ({pagination.total.toLocaleString()} products available)
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 bg-white w-64 shadow-sm"
            />
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.map((p) => (
                <div
                  ...
                  className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs hover:border-medical-400 transition cursor-pointer card-hover flex flex-col justify-between space-y-3"
                >
                  <div>
                    ...
                    <div className="mt-3 p-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-xs">
                      <span className="text-[10px] font-bold uppercase text-slate-400 block">Primary Ingredient</span>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                    ...
                    <span className="text-medical-600 font-semibold flex items-center gap-1">
          ...
          {!search.trim() && pagination.totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 pb-8">
              <span className="text-sm text-slate-500">
                Showing page <strong className="text-navy-900">{pagination.page}</strong> of <strong className="text-navy-900">{pagination.totalPages}</strong>
              </span>
