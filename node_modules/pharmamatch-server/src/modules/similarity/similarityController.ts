...
import { ISimilarityResult, IPharmacyInventory } from '../../types/index.js';
...
export class SimilarityController {
   ...
   */
  public static async findSimilar(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const primaryNorm = NormalizationService.normalizeString(
      ...
      for (const cand of allCandidates) {
        const sim = await SimilarityEngine.calculateSimilarity(targetProduct as any, cand as any, config);
        
        // Fetch candidate's inventory
        const inv = await PharmacyInventory.findOne({ drugProductId: String(cand._id) }).lean();
        sim.inventory = (inv as unknown as IPharmacyInventory) || undefined;
      ...
      await logAudit('SIMILARITY_LOOKUP', req as any, targetProduct.productName, {
        targetProductId: id,
  ...
  public static async compare(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const config = await SimilarityEngine.getActiveConfig();
      const similarity = await SimilarityEngine.calculateSimilarity(target as any, candidate as any, config);
      similarity.inventory = (candInv as unknown as IPharmacyInventory) || undefined;

      await logAudit('PRODUCT_COMPARE', req as any, `${target.productName} vs ${candidate.productName}`, {
  ...
  public static async updateConfig(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const sumWeights = (
        (Number(primaryIngredientWeight) || 0) +
      ...
      config.updatedBy = (req as any).user?.name || 'ADMIN';

      await config.save();

      await logAudit('UPDATE_SIMILARITY_CONFIG', req as any, 'SYSTEM_CONFIG', {
