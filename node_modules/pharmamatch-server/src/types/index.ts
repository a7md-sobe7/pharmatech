...
export interface IActiveIngredient {
  ...
  strength?: number;
  unit?: string;
  normalizedStrengthValue?: number; // In base units (e.g., mg, IU)
...
export interface IDrugProduct {
  ...
  dosageFormCategory?: string; // e.g., 'Oral Solid', 'Oral Liquid', 'Injectable', 'Topical'
  ...
  source: string; // e.g., 'DailyMed', 'FDA', 'EMA', 'Egyptian Drug Authority'
  sourceUrl?: string;
  lastVerifiedAt: Date;
  referencePrice?: number;         // EDA list price
  referencePriceCurrency?: string; // default 'EGP'
  createdAt?: Date;
  updatedAt?: Date;
}
...
export interface ISimilarityConfig {
  _id?: string;
  primaryIngredientWeight: number; // e.g., 0.60
  strengthWeight: number;          // e.g., 0.20
  dosageFormWeight: number;        // e.g., 0.10
  secondaryIngredientWeight: number; // e.g., 0.10
  thresholds: {
    veryHigh: number; // 90
    high: number;     // 75
    moderate: number; // 50
...
export interface IAuditLog {
  ...
  userName?: string;
  userRole?: string;
  action: string; // e.g., 'SEARCH_PRODUCT', 'SIMILARITY_LOOKUP', 'AI_QUERY', 'STOCK_UPDATE'
  target?: string;
