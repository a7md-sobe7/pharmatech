...
export class SimilarityEngine {
  public static DEFAULT_CONFIG: ISimilarityConfig = {
   ...
   */
  public static async getActiveConfig(): Promise<ISimilarityConfig> {
    if (mongoose.connection.readyState === 1) {
      try {
        const config = await SimilarityConfig.findOne().sort({ updatedAt: -1 }).lean();
        if (config) return config as unknown as ISimilarityConfig;
      } catch {
        // ignore
      }
    }
   ...
   */
  public static evaluateDosageForm(targetForm: string, candidateForm: string): { score: number; matched: boolean; explanation: string } {
   ...
   */
  public static evaluateSecondaryIngredients(
    targetProduct: IDrugProduct,
   ...
   */
  public static getSimilarityLevel(score: number, thresholds: ISimilarityConfig['thresholds']): SimilarityLevel {
  ...
  public static async calculateSimilarity(
    targetProduct: IDrugProduct,
    candidateProduct: IDrugProduct,
    customConfig?: ISimilarityConfig
    ...
    const targetPrimaryNorm = NormalizationService.normalizeString(
      targetProduct.primaryActiveIngredient?.normalizedName || targetProduct.primaryActiveIngredient?.name || ''
    );
    const candPrimaryNorm = NormalizationService.normalizeString(
      candidateProduct.primaryActiveIngredient?.normalizedName || candidateProduct.primaryActiveIngredient?.name || ''
    );
    ...
    const strengthRes = StrengthNormalizer.calculateStrengthSimilarity(
      targetProduct.primaryActiveIngredient?.strength,
      targetProduct.primaryActiveIngredient?.unit,
      candidateProduct.primaryActiveIngredient?.strength,
      candidateProduct.primaryActiveIngredient?.unit
    );
    ...
    if (primaryMatched) {
      totalScore = (
        (primaryScore * config.primaryIngredientWeight) +
      ...
      totalScore = (
        (0 * config.primaryIngredientWeight) +
        (strengthScore * 0.05) +
        (dosageScore * 0.05) +
        (secondaryScore * 0.10)
      );
