...
function mapDosageForm(route: string): { dosageForm: string; dosageFormCategory: string; normalizedRoute: string } {
  const r = (route || '').toUpperCase().trim();
  if (r.includes('ORAL.SOLID'))   return { dosageForm: 'Tablets/Capsules', dosageFormCategory: 'Oral Solid',   normalizedRoute: 'Oral' };
  if (r.includes('ORAL.LIQUID'))  return { dosageForm: 'Syrup/Solution',   dosageFormCategory: 'Oral Liquid',  normalizedRoute: 'Oral' };
  if (r.includes('INJECTION'))    return { dosageForm: 'Injection',         dosageFormCategory: 'Parenteral',   normalizedRoute: 'Injection' };
  if (r.includes('TOPICAL'))      return { dosageForm: 'Cream/Ointment',   dosageFormCategory: 'Topical',      normalizedRoute: 'Topical' };
  if (r.includes('EFF'))          return { dosageForm: 'Effervescent',     dosageFormCategory: 'Oral Solid',   normalizedRoute: 'Oral' };
  if (r.includes('SOAP'))         return { dosageForm: 'Soap',             dosageFormCategory: 'Topical',      normalizedRoute: 'Topical' };
  if (r.includes('NASAL'))        return { dosageForm: 'Nasal Spray',      dosageFormCategory: 'Nasal',        normalizedRoute: 'Nasal' };
  if (r.includes('OPHTHALMIC') || r.includes('OCULAR')) return { dosageForm: 'Eye Drops', dosageFormCategory: 'Ophthalmic', normalizedRoute: 'Ophthalmic' };
  if (r.includes('INHALE') || r.includes('INHAL'))      return { dosageForm: 'Inhaler',   dosageFormCategory: 'Inhalation', normalizedRoute: 'Inhalation' };
  if (r.includes('RECTAL'))       return { dosageForm: 'Suppository',      dosageFormCategory: 'Rectal',       normalizedRoute: 'Rectal' };
  if (r.includes('VAGINAL'))      return { dosageForm: 'Vaginal',          dosageFormCategory: 'Vaginal',      normalizedRoute: 'Vaginal' };
  if (r.includes('PATCH'))        return { dosageForm: 'Patch',            dosageFormCategory: 'Transdermal',  normalizedRoute: 'Transdermal' };
...
function parseIngredients(scientificName: string) {
  if (!scientificName?.trim()) {
    return {
      primaryActiveIngredient: { name: 'Unknown', normalizedName: 'unknown', confidence: 'LOW' as const, source: 'EDA' },
      activeIngredients: [],
    };
  }
...
async function importDrugs() {
  ...
  for (let i = 0; i < raw.length; i += BATCH_SIZE) {
    ...
    const docs = batch
      .filter((row: any) => row.commercial_name_en?.trim())  // skip blank names
      .map((row: any) => {
