...
export class ProductController {
    ...
    static async listProducts(req, res, next) {
        try {
            ...
            const therapeuticClass = req.query.therapeuticClass;
    ...
    static async searchProducts(req, res, next) {
        try {
            ...
            await logAudit('SEARCH_PRODUCT', req, queryStr, { normalizedInput });
            ...
            if (results.length === 0 && queryStr.length > 2) {
                results = await DrugProduct.find({ $text: { $search: queryStr } }, { score: { $meta: 'textScore' } })
    ...
    static async getProductById(req, res, next) {
        try {
            ...
            await logAudit('VIEW_PRODUCT_DETAILS', req, String(product._id), { productName: product.productName });
            res.json({
                success: true,
                data: {
                    product,
     ...
     */
    static async getProductSources(req, res, next) {
        try {
            const { id } = req.params;
